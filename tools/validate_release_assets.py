"""Static release-asset validation for the MOMENT-1-base (embeddings, imputation, anomaly scoring) DIMER pipeline.
...
EXPECTED_MODEL_ID = "AutonLab/MOMENT-1-base"
PIPELINE_CLASS = "LoadedMoment"
# Additional 40-hex revisions a document may legitimately cite: the momentfm source commit the
# package installs from (RFC M-7), named in pyproject.toml, README.md and MODEL_CARD.md.
...
_COMMON_CODE = (
    ...
    "\"sequence_length\": config.sequence_length, \"patch_length\": config.patch_length",
...
_COMMON_MD = (
    ...
    "**Upstream vs. this repository.**",
    "raw CSV header",
    "the verdict is `not-measurable`",
    "512-step windows and 8-step non-overlapping patches",
    "is not sent to an external inference service",
)
NOTEBOOKS = {
    "moment_embeddings_colab.ipynb": {
        ...
        "outputs": (
            "outputs/moment_embeddings_input_manifest.json",
            "outputs/moment_embeddings_evaluation_report.json",
            "outputs/moment_embeddings_result.json",
            "outputs/moment_embeddings.csv",
            "outputs/moment_embeddings_provenance.json",
        ),
        "code_markers": _COMMON_CODE + (
            "SAMPLE_SHA256 = \"34fc475828d2f62108b340efd255501a898577be11286c034da2e0f766ee963c\"",
        ...
        "markdown_markers": _COMMON_MD + (
            "**Capability:** pretrained time-series representation extraction",
            "Embeddings are representations, not predictions",
            "**no intrinsic accuracy metric for an embedding vector**",
            "**visible to the encoder**",
            "**per window**",
            "classification, forecasting, anomaly decisions, fine-tuning",
        ),
    },
    "moment_imputation_colab.ipynb": {
        "template": "notebook_template_imputation",
        "profile": "TASK-INFERENCE",
        "model_load": "load_moment(task=\"reconstruction\", weights_dir=WEIGHTS_DIR)",
        "outputs": (
            "outputs/moment_imputation_input_manifest.json",
            "outputs/moment_imputation_evaluation_report.json",
            "outputs/moment_imputation_result.json",
            "outputs/moment_imputed_series.csv",
            "outputs/moment_imputation_metrics.json",
            "outputs/moment_imputation_provenance.json",
        ),
        "code_markers": _COMMON_CODE + (
            "SAMPLE_SHA256 = \"34fc475828d2f62108b340efd255501a898577be11286c034da2e0f766ee963c\"",
        ...
        "markdown_markers": _COMMON_MD + (
            ...
            "`sample-sanity`",
            "**masking unit is the 8-step patch**",
            "**source-missing data from artificial evaluation masking**",
            "**observed values are preserved**",
            "**MAE** is average absolute error",
            "**same artificially withheld positions**",
            "no uncertainty interval is provided",
        ),
    },
    "moment_anomaly_detection_colab.ipynb": {
        "template": "notebook_template_anomaly_detection",
        "profile": "TASK-INFERENCE",
        "model_load": "load_moment(task=\"reconstruction\", weights_dir=WEIGHTS_DIR)",
        "outputs": (
            "outputs/moment_anomaly_detection_input_manifest.json",
            "outputs/moment_anomaly_detection_evaluation_report.json",
            "outputs/moment_anomaly_detection_result.json",
            "outputs/moment_anomaly_scores.csv",
            "outputs/moment_anomaly_provenance.json",
        ),
        "code_markers": _COMMON_CODE + (
            "SAMPLE_SHA256 = \"58855d8961c2b0547e27763ffc95976435180477c2d31f0e217f64bb6dd19c54\"",
            "LABELS_SHA256 = \"f35ec637441c72ee6a16d396c2e76b24de8a19c17ba62e415aefe1bfdc155f3e\"",
            "config = MomentConfig(task=\"reconstruction\")",
        ...
        "markdown_markers": _COMMON_MD + (
            ...
            "`sample-sanity`",
            "**raw reconstruction-residual scoring**, not a binary detector",
            "**higher residual scores mean stronger anomaly evidence according to this score**",
            "**no universal/default threshold in v1**",
...
# Bound per notebook by validate_notebooks (defaults name the primary notebook).
NOTEBOOK_NAME = "moment_embeddings_colab.ipynb"
TEMPLATE_STEM = NOTEBOOKS[NOTEBOOK_NAME]["template"]
EXPECTED_PROFILE = NOTEBOOKS[NOTEBOOK_NAME]["profile"]
MODEL_LOAD_EXPR = NOTEBOOKS[NOTEBOOK_NAME]["model_load"]
EXPECTED_OUTPUTS = NOTEBOOKS[NOTEBOOK_NAME]["outputs"]
CODE_MARKERS = NOTEBOOKS[NOTEBOOK_NAME]["code_markers"]
MARKDOWN_MARKERS = NOTEBOOKS[NOTEBOOK_NAME]["markdown_markers"]
...
def _package_identity() -> tuple[str, str]:
    ...
    template = _load_tool(TEMPLATE_STEM).TEMPLATE
...
def validate_model_card() -> None:
    ...
    _check(
        "## Checkpoint and source provenance" in text,
        "MODEL_CARD.md must carry its '## Checkpoint and source provenance' section (this card's immutable-provenance record)",
    )
...
def _validate_notebook_structure(path: Path, notebook: dict) -> tuple[list[tuple[int, str, ast.Module]], str]:
    ...
    _template = _load_tool(TEMPLATE_STEM).TEMPLATE
...
def validate_notebooks() -> None:
    """Every notebook in NOTEBOOKS, each against its own template and expectations."""
    global NOTEBOOK_NAME, TEMPLATE_STEM, EXPECTED_PROFILE, MODEL_LOAD_EXPR, EXPECTED_OUTPUTS, CODE_MARKERS, MARKDOWN_MARKERS
    tutorials = ROOT / "tutorials"
    notebooks = sorted(tutorials.glob("*.ipynb"))
    _check(
        sorted(p.name for p in notebooks) == sorted(NOTEBOOKS),
        f"the tutorial notebooks must be exactly {sorted(NOTEBOOKS)}, found {[p.name for p in notebooks]}",
    )
    for name, spec in NOTEBOOKS.items():
        NOTEBOOK_NAME = name
        TEMPLATE_STEM = spec["template"]
        EXPECTED_PROFILE = spec["profile"]
        MODEL_LOAD_EXPR = spec["model_load"]
        EXPECTED_OUTPUTS = spec["outputs"]
        CODE_MARKERS = spec["code_markers"]
        MARKDOWN_MARKERS = spec["markdown_markers"]
        _validate_one_notebook(tutorials / name)


def _validate_one_notebook(path: Path) -> None:
    tutorials = ROOT / "tutorials"
